...
def predict():
    try:
        ...
        # Ambil nilai centroid untuk hipertensi dan penyakit jantung
        hypertension_centroid_low = k2_centroids[0][0]
        hypertension_centroid_high = k2_centroids[1][0]
        heart_disease_centroid_low = k2_centroids[0][1]
        heart_disease_centroid_high = k2_centroids[1][1]
        ...
        for i in range(len(features)):
            # Gunakan nilai minimum yang terlihat (0.1) untuk visualisasi jika input 0, dengan anotasi nilai asli
            display_value = 0.1 if input_values[i] == 0 and i in [0, 1] else input_values[i]
            fig = go.Figure()
            ...
            fig.add_trace(go.Bar(x=['Input Anda'], y=[display_value], name='Input Anda', marker_color='red'))
            # Tambahkan anotasi untuk menunjukkan nilai asli jika input 0
            annotation_text = "Tidak (0)" if input_values[i] == 0 and i in [0, 1] else "Ya (1)" if input_values[i] == 1 and i in [0, 1] else f"{input_values[i]:.1f}"
            fig.add_annotation(
                x='Input Anda',
                y=display_value + 0.05,
            ...
            fig.update_layout(
                ...
                yaxis_title='Kemungkinan (0 = Tidak, 1 = Ya)' if i in [0, 1] else 'Nilai',
                barmode='group',
                yaxis=dict(range=[0, 1.2] if i in [0, 1] else [0, max(input_values[i], low_centroid[i], high_centroid[i]) * 1.1]),
                showlegend=True
            )
            bar_charts.append(json.dumps(fig, cls=plotly.utils.PlotlyJSONEncoder))
        dbi_scores = [r['dbi'] for r in results]
        ...
        dbi_fig.update_layout(
            title='Davies-Bouldin Index (DBI) untuk k=2 sampai k=20',
            xaxis_title='Jumlah Klaster (k)',
            yaxis_title='DBI Score',
            showlegend=True
        )
        ...
        silhouette_fig.update_layout(
            title='Silhouette Score untuk k=2 sampai k=20',
            xaxis_title='Jumlah Klaster (k)',
        ...
        # Convert to JSON
        dbi_chart = json.dumps(dbi_fig, cls=plotly.utils.PlotlyJSONEncoder)
        silhouette_chart = json.dumps(silhouette_fig, cls=plotly.utils.PlotlyJSONEncoder)
        return render_template(
            ...
            hypertension_centroid_low=hypertension_centroid_low,
            hypertension_centroid_high=hypertension_centroid_high,
            heart_disease_centroid_low=heart_disease_centroid_low,
            heart_disease_centroid_high=heart_disease_centroid_high,
            hypertension_input=input_data['hypertension'],
            heart_disease_input=input_data['heart_disease']
        )
    except ValueError as e:
        return render_template('form.html', error=f"Error: Masukkan data numerik yang valid. {str(e)}")
